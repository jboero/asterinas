#!/bin/bash
# astrokube-set-fstab.sh — repackage the node image with YOUR /etc/fstab.
#
...
# default table is astrokube/fstab.default — copy it, edit, and pass it here.
#
# Usage:
#   astrokube/astrokube-set-fstab.sh <your-fstab>     # repackage with your fstab
#   astrokube/astrokube-set-fstab.sh --validate <f>   # just sanity-check it
#   astrokube/astrokube-set-fstab.sh --print          # print the current image's fstab
set -euo pipefail
...
[ -f "$CPIO" ] || { echo "!! missing $CPIO — build the image first (astrokube/zero-c-initramfs.sh)" >&2; exit 1; }

WORK=$(mktemp -d /tmp/astrokube-setfstab.XXXXXX)
...
bash astrokube/build-qcow2.sh 2>&1 | grep -iE "QCOW2 ready"

echo "==> done. Boot:  ./run-host-qemu.sh astrokube   (or astrokube-qcow2)"
