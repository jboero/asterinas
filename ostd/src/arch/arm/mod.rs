...
/// Reads the current value of the processor's physical counter (`CNTPCT`).
pub fn read_tsc() -> u64 {
    let lo: u32;
    let hi: u32;
    // `mrrc p15, 0, Rt, Rt2, c14` reads the 64-bit `CNTPCT` into a register pair.
    // SAFETY: Reading `CNTPCT` has no side effects.
    unsafe {
        core::arch::asm!(
