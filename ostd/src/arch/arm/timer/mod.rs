...
//! Timer support backed by the ARM generic timer (the non-secure PL1 physical
//! timer, `CNTP_*` accessed through CP15 c14).
//!
//! On the QEMU `virt` machine the non-secure PL1 physical timer is wired to
...
/// `CNTP_CTL.ENABLE` (bit 0); leaving `IMASK` (bit 1) clear keeps the
/// interrupt unmasked at the timer.
const CNTP_CTL_ENABLE: u64 = 1 << 0;

fn write_cntp_tval(value: u64) {
    // `CNTP_TVAL` is a 32-bit register: `mcr p15, 0, Rt, c14, c2, 0`.
...
fn write_cntp_ctl(value: u64) {
    // `CNTP_CTL` is a 32-bit register: `mcr p15, 0, Rt, c14, c2, 1`.
...
fn get_timebase_freq_raw() -> u64 {
    let freq: u32;
    // `CNTFRQ` is a 32-bit register: `mrc p15, 0, Rt, c14, c0, 0`.
