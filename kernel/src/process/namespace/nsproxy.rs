...
/// `NsProxy` contains all types of namespaces except those that are associated
/// with the `Process` rather than the thread: the user namespace and the PID
/// namespace, which are both held in the `Process` struct.
pub struct NsProxy {
...
/// `CLONE_NEWPID` is also accepted here: the PID namespace lives in the
/// `Process` (not the `NsProxy`), so it is created in the process clone path
/// rather than by `new_clone`.
